class DotsVis {
  ...
  updateVisAll() {
    ...
    vis.dotgroup.transition().attr("visibility", "visible");
      ...
      .attr("cx", function (d) {
        ...
        if (d.show === "Bachelorette") {
          return d3.randomUniform(
            (d.season - 1) * (vis.width / 21) + vis.width / 50,
            d.season * (vis.width / 21)
          )();
        } else {
          return d3.randomUniform(
            (d.season - 1) * (vis.width / 21) + vis.width / 50,
  ...
  updateVisElimSorted() {
    ...
    // Create x-axis
    vis.zoomedgroupXAxis = d3.axisTop().scale(vis.zoomedgroupX);
    ...
    vis.allContestantCircles
      ...
      .on("click", function (d, i) {
        ...
        vis.changeSelectedSeasonColor(
          d.target.__data__.season,
          d.target.__data__.show
        );

        vis.dotgroup.exit().remove();

        document.getElementById("selectedShow").innerText =
          d3.select(this)._groups[0][0].__data__.show;
        document.getElementById("selectedSeason").innerText =
          d3.select(this)._groups[0][0].__data__.season;
      });
  ...
  renderZoomedGroup(season_input, show_input) {
    ...
    // Subtitle
    vis.zoomedgroupText = vis.dotgroup
      .append("text")
    ...
    vis.zoomedgroupWinnerLabel = vis.dotgroup
      .append("text")
    ...
    // Chart title
    vis.zoomedgroupAxisLabel = vis.dotgroup
      .append("text")
    ...
    vis.dotgroup
      ...
      .attr("cx", function (d) {
        if (d.winner != 1) {
          ...
          return vis.zoomedgroupX(10) + 70;
        }
      })
      .attr("cy", function (d) {
        return (
          vis.margin.top + vis.padding + 300 + (d.num_in_elim_week * 270) / 14
        );
      })
      .attr("r", function (d) {
        if (d.winner != 1) {
        ...
        vis.tooltip
          ...
          .style(
            ...
            event.pageY + "px"
                     ...
                     <p style="color: #242635; font-weight: bold;">${d.name}</p>
                     <p style="color: #242635; line-height: 0.5"> Show: ${
                       d.show
                     }</p>
                     <p style="color: #242635; line-height: 0.5"> Season: ${
                       d.season
                     ...
                     <p style="color: #242635; line-height: 0.5"> Age: ${
                       d.age
                     }</p>
                     <p style="color: #242635; line-height: 0.9"> Occupation: ${
                       d.occupation
                     }</p>                
                 </div>\``);

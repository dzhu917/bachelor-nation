class RunnerupVis {
  ...
  initVis() {
    ...
    vis.seasonLabel
      ...
      .attr("y", function (d) {
        if (d > 0) {
          return (
            (vis.height / 5) * (1 + Math.floor((d - 1) / 7)) - vis.margin.bottom
          );
        } else if (d < 0) {
          return (
            (vis.height * 3) / 5 +
            (vis.height / 5) * (1 + Math.floor((-d - 1) / 7)) -
            vis.margin.bottom
          );
        }
      })
    ...
    // Introductory legend (explaining winners and runner ups)
    vis.ruIntroLegend = d3
      .select("#ru-intro-legend")
      .append("svg")
    ...
    // Add legend for first impression rose graphic
    vis.ruFirLegend = d3
      .select("#ru-fir-legend")
      .append("svg")
    ...
    // Add legend for dates graphic
    vis.ruDatesLegend = d3
      .select("#ru-dates-legend")
      .append("svg")
    ...
    // Add axes for circle sizes
    let scale = d3
      .scaleOrdinal()
      .domain(["1 date", "4 dates", "7 dates"])
      .range([0, 75, 150]);
    ...
    // Explain dashed rectangle for winner
    vis.ruDatesLegend
      .append("rect")
      .attr("x", 5)
      .attr("y", 100)
  ...
  updateRunnerup() {
    ...
    vis.runnerupgroup.transition().attr("visibility", "visible");
        ...
        vis.tooltip
          ...
          .style(
            ...
            event.pageY + "px"
                 ...
                 <p style="color: #242635"> ${
                   d.show
                 } season ${d.season} ${d.winner == 1 ? "winner" : "runner-up"}</p>                 
             </div>\``);
      ...
      .attr("cy", function (d) {
        if (d.show === "Bachelorette") {
          if (d.winner == 1) {
            return (
              (vis.height * 3) / 5 +
              (vis.height / 5) * Math.floor((d.season - 1) / 7) +
              (vis.height * 1) / 4 / 5 +
              3
            );
    ...
    // Update highlights for FIR runner ups
    vis.highlights = vis.allContestantDot
      .selectAll(".allHighlights")
      .data(vis.seasons);
  ...
  updateRunnerupFIR() {
    ...
    // Update highlights for date runner ups
    vis.highlights = vis.allContestantDot
      .selectAll(".allHighlights")
      .data(vis.seasons);
